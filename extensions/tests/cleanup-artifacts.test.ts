...
describe("TP-168: Age sweep covers all artifact types", () => {
	...
	it("deletes stale worker-conversation-*.jsonl files", () => {
		const root = createTempRoot();
		createFileWithMtime(join(root, ".pi", "worker-conversation-batch123-lane1.jsonl"), "[]", staleTime);
		createFileWithMtime(join(root, ".pi", "worker-conversation-batch456-lane2.jsonl"), "[]", freshTime);

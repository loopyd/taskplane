...
const VALID_TASK_STATUSES = new Set(["pending", "running", "succeeded", "failed", "stalled", "skipped"]);

const VALID_PERSISTED_MERGE_STATUSES = new Set(["succeeded", "failed", "partial"]);
...
function loadBatchState(repoRoot: string): any | null {
	...
	try {
		...
		throw new StateFileError("STATE_FILE_IO_ERROR", `Failed to read state file: ${(err as Error).message}`);
	}
...
function deleteBatchState(repoRoot: string): void {
	...
	try {
		...
		throw new StateFileError("STATE_FILE_IO_ERROR", `Failed to delete state file: ${(err as Error).message}`);
	}
}
...
function runAllTests() {
	...
	try {
...

		{
			...
			assertEqual(
				parsedAfter.schemaVersion,
				1,
				"on-disk after load: schemaVersion is still 1 (no implicit rewrite)",
			);
...

		{
			...
			assertEqual(
				loaded!.tasks[1].resolvedRepoId,
				"frontend",
				"v2 workspace: task[1].resolvedRepoId is 'frontend'",
			);
...

		{
			...
			assertEqual(
				...
				"round-trip: on-disk schemaVersion is 2 after save",
			);
	...
	function minimalLaneWithRepoTasks(
		laneNum: number,
		tasks: Array<{ taskId: string; promptRepoId?: string; resolvedRepoId?: string }>,
		repoId?: string,
	): any {
		return {
			laneNumber: laneNum,
	...
	function serializeBatchState(
		...
		const mergeResults = (state.mergeResults || []).map((mr: any) => ({
	...
	function persistRuntimeState(
		...
		allTaskOutcomes: any[],
		discovery: {
			pending: Map<string, { taskFolder: string; promptRepoId?: string; resolvedRepoId?: string }>;
		} | null,
		repoRoot: string,
	): void {
		try {
	...
	try {
...

		{
			...
			const outcomes = [minimalOutcome("T-001", "succeeded"), minimalOutcome("T-002", "failed")];
...

		{
			...
			// Cleanup
			try {
				rmSync(blockingRoot, { recursive: true, force: true });
			} catch {
				/* best effort */
			}
		}
...

		{
			...
			const discovery = {
				pending: new Map([["ENRICH-001", { taskFolder: "/my/tasks/ENRICH-001-enrichment" }]]),
			};
			...
			assertEqual(
				loaded!.tasks[0].taskFolder,
				"/my/tasks/ENRICH-001-enrichment",
				"taskFolder enriched from discovery",
			);
		}
...

		{
			...
			const lanes = [
				minimalLaneWithRepoTasks(1, [{ taskId: "WS-001", promptRepoId: "api", resolvedRepoId: "api" }], "api"),
				minimalLaneWithRepoTasks(
					2,
					[{ taskId: "WS-002", promptRepoId: undefined, resolvedRepoId: "frontend" }],
					"frontend",
				),
			];
			const outcomes = [minimalOutcome("WS-001", "succeeded"), minimalOutcome("WS-002", "running")];
			...
			assertEqual(
				ws002.resolvedRepoId,
				"frontend",
				"WS-002 resolvedRepoId serialized from area/default fallback",
			);
...

		{
			...
			const lanes = [
				minimalLaneWithRepoTasks(1, [{ taskId: "WS-010", promptRepoId: "api", resolvedRepoId: "api" }], "api"),
			];
			...
			assertEqual(
				ws020!.resolvedRepoId,
				"frontend",
				"WS-020 resolvedRepoId enriched from discovery (unallocated)",
			);
...

		{
			...
			const lanes = [
				minimalLaneWithRepoTasks(1, [{ taskId: "RT-001", promptRepoId: "api", resolvedRepoId: "api" }], "api"),
			];
...

	{
		...
		const result = analyzeOrchestratorStartupState(["orch-lane-1", "orch-lane-2"], "valid", state, null, new Set());
...

	{
		...
		const result = analyzeOrchestratorStartupState(["orch-lane-1"], "missing", null, null, new Set());
...

	{
		...
		const result = analyzeOrchestratorStartupState([], "missing", null, null, new Set());
...

	{
		...
		const result = analyzeOrchestratorStartupState([], "valid", state, null, new Set());
	...
	function computeResumePoint(persistedState: any, reconciledTasks: any[]): any {
...

	{
		...
		assert(
			typeof gracefulStarting === "string" && gracefulStarting.length > 0,
			"abortGracefulStarting returns string",
		);
		...
		assert(
			typeof gracefulWaiting === "string" && gracefulWaiting.length > 0,
			"abortGracefulWaiting returns string",
		);
		...
		assert(
			typeof gracefulComplete === "string" && gracefulComplete.length > 0,
			"abortGracefulComplete returns string",
		);
	...
	function computeMergeOutcomeForWave(
		...
		const mergeableLaneCount = laneResults.filter((lane) => {
			...
			const hasHardFailure = lane.tasks.some((t) => t.status === "failed" || t.status === "stalled");
			return hasSucceeded && !hasHardFailure;
		}).length;
...

	{
		...
		assertEqual(
			result.persistReasonBeforeCleanup,
			"merge-failure-pause",
			"state persisted with reason merge-failure-pause",
		);
...

	{
		...
		assertEqual(
			result.persistReasonBeforeCleanup,
			"merge-failure-abort",
			"state persisted with reason merge-failure-abort",
		);
		assertEqual(result.errorsAdded.length, 1, "one error added");
	...
	// Verify the source code actually has the cleanup suppression logic
	{
		...
		assert(
			source.includes("if (preserveWorktreesForResume)"),
			"source checks preserveWorktreesForResume in cleanup",
		);
	...
	function parseMergeResultCore(filePath: string): any {
		if (!existsSync(filePath)) {
			throw new TestMergeError("MERGE_RESULT_INVALID", `Merge result file not found: ${filePath}`);
		}

		const raw = readFileSync(filePath, "utf-8").trim();
		if (!raw) {
			throw new TestMergeError("MERGE_RESULT_INVALID", `Merge result file is empty: ${filePath}`);
		}
		...
		return {
			...
			verification: {
				...
				output: typeof parsed.verification.output === "string" ? parsed.verification.output.slice(0, 2000) : "",
			},
		};
	}
	...
	try {
		...
		try {
			rmSync(mergeTestDir, { recursive: true, force: true });
		} catch {
			/* best effort */
		}
	}
...

	{
		...
		try {
			...
			const lanes = [minimalLane(1, ["E2E-001", "E2E-003", "E2E-005"]), minimalLane(2, ["E2E-002", "E2E-004"])];
			const outcomes = [
...

	{
		...
		try {
			...
			assertEqual(
				...
				BATCH_STATE_SCHEMA_VERSION,
				"v1 load path: schemaVersion upconverted to 2",
			);
			...
			assertEqual(
				loaded!.tasks[0].resolvedRepoId,
				undefined,
				"v1 load path: task[0].resolvedRepoId is undefined",
			);
			...
			try {
				rmSync(v1LoadRoot, { recursive: true, force: true });
			} catch {
				/* best effort */
			}
		}
...

	{
		...
		try {
			...
			try {
				rmSync(v1NoRewriteRoot, { recursive: true, force: true });
			} catch {
				/* best effort */
			}
		}
...

	{
		...
		try {
			...
			try {
				rmSync(v1SaveRoot, { recursive: true, force: true });
			} catch {
				/* best effort */
			}
		}
...

	{
		...
		try {
			...
			assertEqual(
				loaded!.tasks[0].resolvedRepoId,
				undefined,
				"v2 repo-mode load: task resolvedRepoId is undefined",
			);
			assertEqual(loaded!.lanes[0].repoId, undefined, "v2 repo-mode load: lane repoId is undefined");
		} finally {
			try {
				rmSync(v2RepoRoot, { recursive: true, force: true });
			} catch {
				/* best effort */
			}
		}
...

	{
		...
		try {
			...
			assertEqual(
				loaded!.schemaVersion,
				BATCH_STATE_SCHEMA_VERSION,
				"v2 workspace-mode load: schemaVersion is 2",
			);
			...
			assertEqual(
				loaded!.tasks[0].resolvedRepoId,
				"api",
				"v2 workspace-mode load: task[0].resolvedRepoId is 'api'",
			);
			...
			assertEqual(
				loaded!.tasks[1].resolvedRepoId,
				"frontend",
				"v2 workspace-mode load: task[1].resolvedRepoId is 'frontend'",
			);
			...
			try {
				rmSync(v2WsRoot, { recursive: true, force: true });
			} catch {
				/* best effort */
			}
		}
...

	{
		...
		try {
			...
			try {
				rmSync(futureVersionRoot, { recursive: true, force: true });
			} catch {
				/* best effort */
			}
		}
...

	{
		...
		try {
			...
			try {
				rmSync(v0Root, { recursive: true, force: true });
			} catch {
				/* best effort */
			}
		}
...

	{
		...
		try {
			...
			assertEqual(
				resumePoint.resumeWaveIndex,
				0,
				"v1 resume: wave 0 (TS-002 mark-failed NOT done for wave-skip)",
			);
			...
			try {
				rmSync(v1ResumeRoot, { recursive: true, force: true });
			} catch {
				/* best effort */
			}
		}
...

	{
		...
		assert(
			point.failedTaskIds.includes("WS-003"),
			"WS-003 in failed (mark-failed: dead session + no DONE + no worktree)",
		);
	}
...

	{
		...
		assertEqual(
			ws002!.action,
			"mark-failed",
			"workspace: WS-002 mark-failed (dead session, no .DONE, no worktree)",
		);
	...
	function buildTestDepGraph(deps: Record<string, string[]>): {
...

	{
		...
		assertEqual(
			point.resumeWaveIndex,
			0,
			"all-failed-wave: resumes from wave 0 (mark-failed is NOT done for wave-skip)",
		);
...

	{
		...
		const wave1BlockedCount = state.wavePlan[1].filter((taskId: string) => blockedTaskIds.has(taskId)).length;
...

	{
		...
		const state = minimalPersistedState({
			wavePlan: [["T1"]],
			tasks: [makeTaskRecord({ taskId: "T1", status: "running" })],
		});
	...
	{
		console.log(
			"  ▸ reconstructAllocatedLanes: preserves laneNumber, laneId, branch, repoId from persisted records",
		);
		const persistedLanes = [
			{
				laneNumber: 1,
	...
	// 2.3: collectAllRepoRoots merges roots from multiple sources
	{
		...
		const persistedLanes = [{ repoId: "api" as string | undefined }, { repoId: "frontend" as string | undefined }];
	...
	{
		console.log(
			"  ▸ checkpoint attribution: lanes[] and tasks[].repoId survive resume-reconciliation → wave-execution-complete",
		);
				...
				tasks: [
					{
						taskId: "T2",
						order: 0,
						task: { promptRepoId: "frontend", resolvedRepoId: "frontend" },
						estimatedMinutes: 5,
					},
				],
				strategy: "round-robin",

...
export async function resumeOrchBatch(
	...
	if (isForceResume) {
		onNotify(ORCH_MESSAGES.forceResumeStarting(persistedState.batchId, persistedState.phase), "warning");
	...
	const resumeBackend: RuntimeBackend = selectRuntimeBackend("all", runtimeWavePlan, workspaceConfig).backend;
	...
	if (reExecuteTasks.length > 0) {
		onNotify(`🔄 Re-executing ${reExecuteTasks.length} interrupted task(s) in existing worktrees...`, "info");
	...
	const persistedStatusByTaskId = new Map(persistedState.tasks.map((task) => [task.taskId, task.status] as const));
	...
	for (let waveIdx = resumePoint.resumeWaveIndex; waveIdx < wavePlan.length; waveIdx++) {
		...
		if (waveTasks.length === 0) {
			...
			if (resumePoint.mergeRetryWaveIndexes.includes(waveIdx)) {
				...
				const syntheticLaneResults: LaneExecutionResult[] = mergeRetryLanes.map((lane) => {
					...
					const laneHasHardFailure = laneTasks.some((t) => t.status === "failed" || t.status === "stalled");
					const laneHasSucceeded = laneTasks.some((t) => t.status === "succeeded");
					const overallStatus = laneHasHardFailure ? (laneHasSucceeded ? "partial" : "failed") : "succeeded";
		...
		for (const taskId of waveResult.failedTaskIds) {
			...
			const taskRecord = laneForTask?.tasks.find((task) => task.taskId === taskId)?.task;
			...
			const segmentId =
				outcome?.segmentId ?? taskRecord?.activeSegmentId ?? segmentFrontier?.activeSegmentId ?? undefined;
